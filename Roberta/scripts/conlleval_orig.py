...
class Eval:
    ...
    def is_chunk_end(self, prev_tag, tag):
        ...
        """
        prefix1, chunk_type1 = self.split_tag(prev_tag)
        prefix2, chunk_type2 = self.split_tag(tag)
    ...
    def count_chunks(self, true_seqs, pred_seqs):
        ...
        for true_tag, pred_tag in zip(true_seqs, pred_seqs):
            ...
            _, true_type = self.split_tag(true_tag)
            _, pred_type = self.split_tag(pred_tag)

            if correct_chunk is not None:
                true_end = self.is_chunk_end(prev_true_tag, true_tag)
                pred_end = self.is_chunk_end(prev_pred_tag, pred_tag)
            ...
            true_start = self.is_chunk_start(prev_true_tag, true_tag)
            pred_start = self.is_chunk_start(prev_pred_tag, pred_tag)
    ...
    def get_result(self, correct_chunks, true_chunks, pred_chunks,
        ...
        # compute overall precision, recall and FB1 (default values are 0.0)
        prec, rec, f1 = self.calc_metrics(sum_correct_chunks, sum_pred_chunks, sum_true_chunks)
        res = (prec, rec, f1)
        if not verbose:
            return res
